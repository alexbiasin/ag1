...
class Player(pygame.sprite.Sprite):
    ...
    def cycleImage(self):
        ...
        self.index += 1
        if self.index >= len(self.images[self.direction.value]):
            self.index = 0        
...
def procesarComando(comando):
    ...
    #some fun
        ...
        globalMessage(randomString(['There is no time to be wasting.','Walking is good for your health.','Nah, I don\'t wan\'t to sit, thanks.']))
    elif words[0] == 'sleep':
        ...
        if (item2 in inventory.keys()): # mezclar 2 items del inventory
            ...
            if ('locked' in rooms[currentRoom]['items'][item2]):
                if (rooms[currentRoom]['items'][item2]['locked'] == True):
                    if (item1 == rooms[currentRoom]['items'][item2]['unlockeritem']):
                        ...
                        if ('iteminside' in rooms[currentRoom]['items'][item2]):
                            ...
                            globalMessage(rooms[currentRoom]['items'][item2]['unlockingtext'])
                        else:
                            globalMessage('OJO: el iteminside no esta en '+currentRoom) # no debiera llegar aca
                    else:
                        globalMessage('I think the '+item1+' is not meant to be used with the '+item2+'.')
                else:
                    globalMessage(randomString(['Not again!','You\'ve already done that.','Don\'t be repetitive dude!']))
                ...
                if (rooms[currentRoom]['items'][item2]['blocked'] == True):
                    ...
                    globalMessage(randomString(['Are you still seeing that?','You\'ve already done that.','Don\'t be repetitive pal!']))
            else:
                globalMessage(randomString(['Can\'t use ' + item1 + ' with ' + item2 + '!','I don\'t think the '+item1+' is meant to be used with the '+item2,'...'+item1+' with '+item2+' does not compute.']))
        else:
...
#    global player
    ...
    x = relativeW(coords[0])
    y = relativeH(coords[1])
...
def draw_layers():
    if ('layers' in rooms[currentRoom].keys()):
        ...
        for layer in layers:
            ...
            z = relativeH( layers[layer]['z'] )
            xfrom = relativeW( layers[layer]['xfrom'] )
            xto = relativeW( layers[layer]['xto'] )
